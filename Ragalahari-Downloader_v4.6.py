# some enhaced features like 
# Retry Logic 
# Folder name validation 
# Concurrent downloading
# inproved progress bar
# error handling
import os
import re
import requests
...
class RagaDownloader:
    ...
    def download_images(self, image_urls):
        ...
        if not self.folder_path:
            ...
            # Validate folder name to avoid issues with OS file system
    ...
    def download_image(self, url, retries=3, delay=3):
        try:
            headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
            }
            ...
            if retries > 0:
                console.print(f"[bold yellow]Retrying ({retries} attempts left) for {url}...[/bold yellow]")
                sleep(delay)  # Delay before retrying
                return self.download_image(url, retries - 1, delay)  # Retry logic
            else:
